      ...
      <Modal v-if="modalOpen && SELECTD_PLANET">
        ...
        <template slot="body">
          <div class="u-flex u-flex-column u-flex-align-start">
            <p><label>Diameter:</label> {{ SELECTD_PLANET.diameter }}</p>
            <p><label>Climate:</label> {{ SELECTD_PLANET.climate }}</p>
            <p><label>Population:</label> {{ SELECTD_PLANET.population }}</p>
          </div>
        </template>
...
export default {
  ...
  computed: {
    ...mapGetters("personList/", ["PERSON_LIST", "SELECTD_PLANET"]),
    rows() {
  ...
  methods: {
    ...mapActions("personList/", ["GET_PERSON_LIST", "GET_PLANET_DATA"]),
    openModal(cellData) {
